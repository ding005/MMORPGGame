...
namespace XLua
{
    public partial class DelegateBridge : DelegateBridgeBase
    {
		...
		public void __Gen_Delegate_Imp15(byte[] p0)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public void __Gen_Delegate_Imp16(UnityEngine.Transform p0, object p1)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public void __Gen_Delegate_Imp17(YouYou.HttpCallBackArgs p0)
		{
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public override Delegate GetDelegateByType(Type type)
		{
			...
			{
			    return new YouYou.BaseAction(__Gen_Delegate_Imp7);
			}
		
		    if (type == typeof(YouYou.LuaForm.OnCloseHandler))
			{
			    return new YouYou.LuaForm.OnCloseHandler(__Gen_Delegate_Imp7);
			}
		
		    if (type == typeof(YouYou.LuaForm.OnBeforDestroyHandler))
			{
			    return new YouYou.LuaForm.OnBeforDestroyHandler(__Gen_Delegate_Imp7);
			}
			...
			{
			    return new YouYou.CommonEvent.OnActionHandler(__Gen_Delegate_Imp14);
			}
		
		    if (type == typeof(YouYou.LuaForm.OnOpenHandler))
			{
			    return new YouYou.LuaForm.OnOpenHandler(__Gen_Delegate_Imp14);
			}
		
		    if (type == typeof(YouYou.SocketEvent.OnActionHandler))
			{
			    return new YouYou.SocketEvent.OnActionHandler(__Gen_Delegate_Imp15);
			}
		
		    if (type == typeof(YouYou.LuaForm.OnInitHandler))
			{
			    return new YouYou.LuaForm.OnInitHandler(__Gen_Delegate_Imp16);
			}
		
		    if (type == typeof(YouYou.HttpSendDataCallBack))
			{
			    return new YouYou.HttpSendDataCallBack(__Gen_Delegate_Imp17);
			}
